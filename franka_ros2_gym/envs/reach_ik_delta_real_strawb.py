...
class ReachIKDeltaRealStrawbEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        pose.orientation.x = float(self.initial_orientation[1])
        pose.orientation.y = float(self.initial_orientation[2])
        pose.orientation.z = float(self.initial_orientation[3])
        pose.orientation.w = float(self.initial_orientation[0])
        ...
        while (pos_diff > 0.01) or (rot_diff < 0.99):
            current_pos = np.array([self.x, self.y, self.z])
            current_orientation = np.roll(Rotation.from_matrix(self.rot_mat).as_quat(), 1)
            pos_diff = np.linalg.norm(current_pos - self.initial_position)
            rot_diff = np.abs(np.dot(current_orientation, self.initial_orientation))
            self.goal_pose_pub.publish(pose)
            self.gripper_pub.publish(Float32(data=-1.0))
    ...
    def step(self, action):
        ...
        current_rotation_quat = current_rotation.as_quat()
        current_rotation_quat = np.roll(current_rotation_quat, 1)
        current_rotation = Rotation.from_quat(current_rotation_quat)
        ...
        pose.orientation.x = float(final_quat[1])
        pose.orientation.y = float(final_quat[2])
        pose.orientation.z = float(final_quat[3])
        pose.orientation.w = float(final_quat[0])
