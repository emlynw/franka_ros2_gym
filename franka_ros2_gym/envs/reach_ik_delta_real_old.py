...
class ReachIKDeltaRealEnv(gym.Env):
        ...
        **kwargs
        ...
        if image_obs:
            self.observation_space["images"] = Dict({
                "wrist1": Box(0, 255, shape=(self.height, self.width, 3), dtype=np.uint8),
                "wrist2": Box(0, 255, shape=(self.height, self.width, 3), dtype=np.uint8),
                "front": Box(0, 255, shape=(self.height, self.width, 3), dtype=np.uint8),
            })
            if self.depth:
                self.observation_space["images"]["wrist1_depth"] = Box(0, 65535, shape=(self.height, self.width), dtype=np.uint16)
                self.observation_space["images"]["wrist2_depth"] = Box(0, 65535, shape=(self.height, self.width), dtype=np.uint16)
                self.observation_space["images"]["front_depth"] = Box(0, 65535, shape=(self.height, self.width), dtype=np.uint16)
        ...
        self.wrist2 = None
        self.front = 1
        self.wrist1_depth = None
        self.wrist2_depth = None
        self.front_depth = 1
        self.rot_mat = None
        self.x = None
        self.y = None
        ...
        self.initialized = [self.wrist1, self.wrist2, self.front, self.wrist1_depth, self.wrist2_depth,
                            self.front_depth, self.rot_mat, self.x, self.y, self.z, self.gripper_width]
    ...
    def ros_setup(self):
        ...
        self.wrist1_sub = self.node.create_subscription(
            Image,
            '/camera1/camera1/color/image_raw',
            self.wrist1_callback,
            qos_profile=custom_qos_profile,
            callback_group=self.callback_group
        )
        self.wrist1_depth_sub = self.node.create_subscription(
            Image,
            '/camera1/camera1/aligned_depth_to_color/image_raw',
            self.wrist1_depth_callback,
            qos_profile=custom_qos_profile,
            callback_group=self.callback_group
        )
        self.wrist2_sub = self.node.create_subscription(
            Image,
            '/camera2/camera2/color/image_raw',
            self.wrist2_callback,
            qos_profile=custom_qos_profile,
            callback_group=self.callback_group
        )
        self.wrist2_depth_sub = self.node.create_subscription(
            Image,
            '/camera2/camera2/aligned_depth_to_color/image_raw',
            self.wrist2_depth_callback,
            qos_profile=custom_qos_profile,
            callback_group=self.callback_group
        )
        self.front_sub = self.node.create_subscription(
            Image,
        ...
        self.front_depth_sub = self.node.create_subscription(
            Image,
            '/camera3/camera3/aligned_depth_to_color/image_raw',
    ...
    def reset(self, seed=None, options=None):
        ...
        while not (self.wrist1 and self.wrist2 and self.front and self.wrist1_depth and self.wrist2_depth and 
                   self.front_depth and self.rot_mat and self.x and self.y and self.z and self.gripper_width):
    ...
    def step(self, action):
        ...
        if not (self.wrist1 and self.wrist2 and self.front and self.wrist1_depth and self.wrist2_depth and 
                   self.front_depth and self.rot_mat and self.x and self.y and self.z and self.gripper_width):
    ...
    def _get_obs(self):
        # Wait for fresh state
        while any (value is None for value in self.initialized):
            rclpy.spin_once(self.node, timeout_sec=0.01)
